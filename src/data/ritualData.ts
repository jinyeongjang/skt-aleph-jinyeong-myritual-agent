...
export const RESUME_AND_PORTFOLIO: ResumeAndPortfolio = {
  studentName: '장진영',
  firstSentence:
    '실패의 자리에서 원인을 파고들고, 일상의 단단한 루틴으로 안전한 인프라와 팀의 신뢰를 완성하는 개발자 장진영입니다.',
  resumeBody: `나는 초기 웹 서비스 인프라 배포 프로젝트에서 예상치 못한 장애를 마주하며 뼈아픈 고비를 겪었습니다. 원인을 모른 채 임시 코드를 덧대다 정해진 시연을 맞추지 못했고, 기본기의 부재가 팀 전체에 미친 무게를 깊이 체감했습니다. 하지만 그 실패는 나를 멈추게 하는 대신 일하는 기준을 완전히 바꾸는 전환점이 되었습니다. "원리를 모르는 블랙박스 코드는 쓰지 않는다", "매일 사전 점검 루틴을 지킨다"는 원칙을 삶의 규칙으로 세웠고, 이 회복의 힘은 SKT ALEPH 1기 과정에서 세 가지 핵심 강점으로 증명되었습니다.

첫째, 원리 탐구와 집요한 기술 문제 해결력입니다. 2026년 8월 26일 실습 중 발생한 코드 에러 앞에서 포기하지 않고 밑단 로그를 끝까지 파고들어 원인을 규명했습니다. 과제 8 WebAuthn 패스키 인증을 구축할 때도 브라우저 서명 포맷(DER)과 Web Crypto API 간의 바이트 구조 불일치 문제를 W3C 공식 명세를 분석하여 직접 바이트 파서를 구현해 해결했습니다.

둘째, 온기 있는 소통과 배움 나눔의 조력 리더십입니다. 2026년 8월 12일 kali 리눅스 설치가 지연되던 동료들을 위해 다양한 해결책을 적용해 환경 셋팅을 완결했고, 2026년 9월 17일 조별 과제에서는 팀원이 대본 없이도 발표할 수 있도록 직관적인 자료 구조를 설계했습니다. 2년 10개월간의 SW·AI 교육 강사 및 매니저 경험을 살려 팀에 배움의 온기와 심리적 안전감을 더했습니다.

셋째, 루틴 기반의 꾸준한 실행력과 회복탄력성입니다. 2026년 8월 24일 자격증 시험과 먼 거리 통학이 겹치는 일정 속에서도 매일 아침 강의장에 1등으로 도착해 하루 목표를 점검하고 당일 복습을 실천했습니다. 감정이나 상황에 흔들리지 않고 30일간 정돈된 아침 리추얼을 완주하며 지속 가능한 성장의 토대를 다졌습니다.`,
  lastSentence:
    '어떤 복잡한 장애 앞에서도 바닥부터 원리를 파고드는 집요함과 지속 가능한 루틴으로, 팀이 믿고 맡길 수 있는 가장 견고한 보안·인프라 환경을 구축하겠습니다.',
  portfolioFramework: [
    {
